...
deleteButton.addEventListener("click", function () {
    ...
    const result = confirm("本当にこの単語を削除しますか？");
...
bulkAddButton.addEventListener("click", function () {
    ...
    const lines = bulkWords.value.trim().split("\n");
    ...
    lines.forEach(function (line) {
        ...
        const level = Number(parts[2].trim());

        if (word === "" || answer === "" || isNaN(level)) {
            return;
        }
...
unknownButton.addEventListener("click", function () {
    ...
    currentWord.unknown = !currentWord.unknown;
...
const wordbookMenuButton =
    document.querySelector("#menuList button:first-child");

wordbookMenuButton.addEventListener("click", function () {
...
const wordbookScreen = document.getElementById("wordbookScreen");
